...
#define StopClassifierPath "/home/rtrk/myROSworkspace/src/bachelor/classifiers/classifier/cascade.xml"
...
void setRedHueFrame(const cv::Mat &inputArg, cv::Mat &outputArg)
{
    ...
    {
        cv::Mat tmp = outputArg.clone();
        cv::resize(tmp,tmp,cv::Size(tmp.cols*0.5, tmp.rows*0.5) );
        cv::imshow("red hue", tmp);
    }
}
...
/*
    cv::Mat tmp = frameArg.clone();
    cv::resize(tmp,tmp,cv::Size(tmp.cols*0.5, tmp.rows*0.5) );
    cv::imshow("original frame", tmp);
*/    
}
