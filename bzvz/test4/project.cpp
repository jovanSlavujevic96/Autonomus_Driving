...
void trainDigitClassifier(void)
{
    ...
    cv::threshold(gray,thr, 125, 255, cv::THRESH_BINARY_INV); //Threshold to find contour
    cv::imshow("ci",thr);
    cv::waitKey(0);
    thr.copyTo(con);
    ...
    {
        cv::Rect r= cv::boundingRect(contours[i]); //Find bounding rect for each contour
    ...
    cv::FileStorage Data;
    {
        ...
        Data = cv::FileStorage(ss.str(), FileStorage::WRITE); // Store the sample data in a file
    }
    Data << "data" << sample;
    Data.release();
    ...
    cv::FileStorage Label; 
    {
        ...
        Label = cv::FileStorage(ss.str(), FileStorage::WRITE); // Store the label data in a file
    }
