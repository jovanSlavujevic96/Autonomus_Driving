...
void StopSignProcessor::loadCascade(cv::CascadeClassifier *cascade, const int size, const std::string *path)
{
...
int StopSignProcessor::resize(cv::Mat &image, const int limit)
{
    int numOfResizing = 0;
...
void StopSignProcessor::crop(cv::Mat &image)
{
    image = image(cv::Rect(0, 0, image.cols, std::round(image.rows/2))).clone();
}

void StopSignProcessor::redColorSegmentation(const cv::Mat &sample, cv::Mat &result)
{
    cv::Mat hsv;
...
std::vector<cv::Rect> StopSignProcessor::getRedContours(const cv::Mat1b &hueImage) const
{    
    cv::Mat blured;
...
std::vector<cv::Mat> StopSignProcessor::getTextImagesForOCR(const int numOfResizing, std::vector<cv::Rect> &contours)
{   
    std::vector<cv::Mat> tmpVec;
    cv::Mat tmpFrame = m_Frame.clone();
    StopSignProcessor::redColorSegmentation(tmpFrame, tmpFrame);
...
std::vector<bool> StopSignProcessor::getDetectionFromOCR(const std::vector<cv::Mat> &images)
{
...
void StopSignProcessor::setFrame(sensor_msgs::Image &rawFrame)
{
    ...
    auto numOfResizing = StopSignProcessor::resize(helpImage, 600); //resize image for faster performance!
    StopSignProcessor::crop(helpImage); //use only upper half of the image(frame) for faster performance!
